...
function definirVisibilidadeBotao(el, visivel) {
  if (!el) return;
...
function renderizarMinigame() {
  ...
  definirVisibilidadeBotao(document.getElementById('btn-finalizar-mg'), false);
...
function mostrarFeedbackMG(ok, mostrarProximo = true) {
  ...
  if (mostrarProximo) {
    ...
    definirVisibilidadeBotao(btnProx, !isUltimo);
    definirVisibilidadeBotao(btnFin, isUltimo);
    if (isUltimo && btnFin) btnFin.textContent = 'Ver Resultado 🏆';
    else if (btnProx) btnProx.textContent = 'Próximo Jogo →';
  } else if (zerouVidas) {
    definirVisibilidadeBotao(document.getElementById('btn-proximo-mg'), false);
    definirVisibilidadeBotao(document.getElementById('btn-finalizar-mg'), false);
  }
