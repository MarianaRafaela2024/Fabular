...
function mapStorySummaryToLegacy(story) {
  ...
  return {
    ...
    cena: story.cena.toUpperCase() || story.Cena.toUpperCase() || '🌟',
...
async function carregarDetalheHistoriaDaApi(id) {
  ...
  return {
    ...
    titulo: data.titulo,
    emoji: data.emoji || '📖',
    cena: data.cena.toUpperCase() || '🌟',
    duracao: data.duracao || '5 min',
    textoCompleto: texto,
    texto: texto,
    fases: texto ? [{ texto, cena: data.cena.toUpperCase() || '🌟' }] : [],
...
/* =========================================================================
   Imagens de capa e cena — só histórias manuais (IDs 1 a 11)
   Histórias da IA não entram nestes mapas e continuam com emoji.
   ========================================================================= */

const CAMINHO_BASE_CENA = 'midia/cena/';

/** Um JPG por história: serve na capa da biblioteca e na cena da leitura. */
const MAPA_IMAGEM_POR_ID = {
  '1': 'leao',
  '2': 'menina',
  '3': 'biblioteca',
  '4': 'chuva',
...
function idHistoriaParaImagem(historia) {
  const raw = historia && typeof historia === 'object' ? historia.id : historia;
  return String(normalizarIdHistoria(raw) || '');
}

function caminhoImagemHistoria(slug) {
  return slug ? `${CAMINHO_BASE_CENA}${slug}.jpg` : null;
}

function resolverImagemHistoria(historia) {
  return caminhoImagemHistoria(MAPA_IMAGEM_POR_ID[idHistoriaParaImagem(historia)]);
}

function resolverImagemCapa(historia) {
  return resolverImagemHistoria(historia);
}

function resolverImagemCena(historia) {
  return resolverImagemHistoria(historia);
}
