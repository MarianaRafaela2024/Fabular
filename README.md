...
- páginas: `qmsomos.html` (institucional), `login.html` (criança + responsável/perfis) e `index.html` (aplicação principal);
...
3. **Login/cadastro do responsável** (primeiro acesso no dispositivo):
...
5. **App principal** (`index.html` + `app.js`):
...
6. **Relatório local para o responsável**:
...
- `login.html` — login da criança, login/cadastro do responsável e seleção de perfil.
- `index.html` — shell da aplicação principal.
- `login.js` — lógica de perfil infantil e fluxo responsável/perfis em `localStorage`.
